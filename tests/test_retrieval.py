...
async def main():
    ...
    try:
        ...
        for idx, res in enumerate(child_results):
            print(f"    Rank {idx+1}: ID={res.chunk_id} | Score={res.score} | Source={res.source} | Text='{res.text[:40]}...'")
            
        assert len(child_results) > 0
        # Check that we retrieved correct chunk IDs
        assert any(r.chunk_id == "child_1" for r in child_results) or any(r.chunk_id == "child_2" for r in child_results)
        ...
        for idx, cand in enumerate(parent_candidates):
            print(f"    Candidate {idx+1}: ID={cand.parent_chunk.id} | RRF Score={cand.rrf_score} | Text='{cand.parent_chunk.text[:50]}...'")
        ...
        print(f"  Reranked top contexts:")
        for idx, ctx in enumerate(reranked_contexts):
            print(f"    Rank {ctx.final_rank}: ID={ctx.parent_chunk.id} | LLM Score={ctx.llm_relevance_score} | Text='{ctx.parent_chunk.text[:50]}...'")
        ...
        for idx, ctx in enumerate(contexts):
            print(f"    Result {idx+1}: ID={ctx.parent_chunk.id} | Score={ctx.llm_relevance_score} | Text='{ctx.parent_chunk.text[:50]}...'")
