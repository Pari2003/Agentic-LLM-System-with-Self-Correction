...
def main():
    ...
    try:
        ...
        print(
            f"  Document successfully saved and retrieved: '{retrieved_doc.title}' by {retrieved_doc.authors}"
        )
        ...
        assert retrieved_parent.section_title == "Introduction", (
            "Parent chunk section title mismatch"
        )
        ...
        query_results = vector_store.query_similarity(
            query_vector=[0.11] * 768, session_id=session_id, top_k=2
        )
        assert len(query_results) > 0, "No results returned from ChromaDB"
        assert query_results[0].chunk_id == "child_1", (
            f"Incorrect top result: {query_results[0].chunk_id}"
        )
        # Score calculation is 1 - distance. Distances are small, so score should be close to 1.0.
        print(
            f"  Query result: Chunk ID={query_results[0].chunk_id}, Score={query_results[0].score:.4f}, Text='{query_results[0].text}'"
        )

        print("Testing isolation from other sessions...")
        other_query_results = vector_store.query_similarity(
            query_vector=[0.11] * 768, session_id="different_session_id", top_k=2
        )
        assert len(other_query_results) == 0, (
            f"Query returned chunks from another session: {other_query_results}"
        )
        ...
        assert doc_store.get_session(expired_session_id) is None, (
            "Expired session SQLite record was not purged"
        )
        ...
        assert doc_store.get_session(session_id) is not None, (
            "Original session not found before close"
        )
        ...
        assert doc_store.get_parent_chunk(parent_id) is None, (
            "Parent chunk was not cascadingly deleted"
        )
        ...
        print(
            "  Manual session close and cascading deletes verified (SQLite, ChromaDB, and Neo4j)."
        )
