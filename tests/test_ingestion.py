...
async def main():
    ...
    for page_num, c_type, content, section in parsed_items:
        print(f"  Page {page_num} | Type: {c_type.value} | Length: {len(content)} characters | Section: {section}")
    ...
    print(f"Generated {len(group_embeddings)} embeddings of dimension {len(group_embeddings[0]) if group_embeddings else 0}")
    ...
    for idx, parent in enumerate(parent_chunks):
        print(f"  Parent {idx+1} ({parent.id}): '{parent.text[:80]}...' contains children: {parent.child_ids}")
    ...
    print(f"First child chunk vector sample (first 5 elements): {embedded_children[0].embedding[:5]}")
